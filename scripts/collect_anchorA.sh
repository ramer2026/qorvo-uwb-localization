...
POINT="${1:?Usage: collect_anchorA.sh POINT CONDITION [SECONDS]}"
COND="${2:?Usage: collect_anchorA.sh POINT CONDITION [SECONDS]}"
DURATION="${3:-60}"

ANCHOR="anchorA"
PORT="ftdi://FT4222/V252101134A"
SESSION_ID="45"
...
echo "============================================================"
echo "Collecting $ANCHOR / CONTROLLER"
echo "Point:      $POINT"
echo "Condition:  $COND"
...
"$RUN_FIRA_TWR" -p "$PORT" -s "$SESSION_ID" -t "$DURATION" \
  --en-rssi \
  --en-diag \
  --stats \
  --diag_dump \
